...
void pinhole_Translate(int*** world, int height, int width, int** image_out)
{
	...
	for (int y = 0; y < height; y++)
		for (int x = 0; x < width; x++)
		{
			int px = (int)(-x / ratio) + X_MOVE;
			int py = (int)(-y / ratio) + Y_MOVE;

			double dpx = (double)(-x / ratio) + X_MOVE;
			double dpy = (double)(-y / ratio) + Y_MOVE;

			double fx1 = (dpx - px) / X_MOVE;
			double fx2 = (1 - fx1);
			double fy1 = (dpy - py) / Y_MOVE;
			double fy2 = (1 - fy1);
